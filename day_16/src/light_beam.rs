...
impl Map {
    fn parse_from_input(input: &str) -> Self {
        ...
        for line in input.lines() {
            ...
            for char in line.chars() {
                row.push(match char {
                    '.' => Tile::Empty { is_energized: false },
                    '/' => Tile::MirrorLeftUp { is_energized_from_left_up: false, is_energized_from_right_down: false },
                    '\\' => Tile::MirrorLeftDown { is_energized_from_left_down: false, is_energized_from_right_up: false },
                    '|' => Tile::SplitterUpDown { is_energized: false },
                    '-' => Tile::SplitterLeftRight { is_energized: false },
            ...
            if let Some((direction, mut x, mut y)) = light_sources.pop() {
                ...
                match direction {
                    Direction::Left => loop {
                        ...
                        match current_tile {
                            Tile::Empty { is_energized } | Tile::SplitterLeftRight { is_energized } => *is_energized = true,
                            Tile::MirrorLeftUp { is_energized_from_right_down, .. } => {
                            ...
                            Tile::MirrorLeftDown { is_energized_from_right_up, .. } => {
                    ...
                    Direction::Right => loop {
                        ...
                        match current_tile {
                            Tile::Empty { is_energized } | Tile::SplitterLeftRight { is_energized } => *is_energized = true,
                            Tile::MirrorLeftUp { is_energized_from_left_up, .. } => {
                            ...
                            Tile::MirrorLeftDown { is_energized_from_left_down, .. } => {
                    ...
                    Direction::Up => loop {
                        ...
                        match current_tile {
                            Tile::Empty { is_energized } | Tile::SplitterUpDown { is_energized } => *is_energized = true,
                            Tile::MirrorLeftUp { is_energized_from_right_down, .. } => {
                                if !*is_energized_from_right_down {
                                    // this mirror has never been energized from this angle
                                    *is_energized_from_right_down = true;
                            ...
                            Tile::MirrorLeftDown { is_energized_from_left_down, .. } => {
                                if !*is_energized_from_left_down {
                                    // this mirror has never been energized from this angle
                                    *is_energized_from_left_down = true;
                                    light_sources.push((Direction::Left, x.overflowing_sub(1).0, y));
                                }
                                break;
                            },
    ...
    fn get_total_energized_tiles(self) -> usize {
        self.tiles.into_iter().flatten().filter(|tile| match tile {
            Tile::Empty { is_energized } => *is_energized,
            Tile::MirrorLeftUp { is_energized_from_left_up: is_energized_from_left, is_energized_from_right_down: is_energized_from_right } => *is_energized_from_left || *is_energized_from_right,
            Tile::MirrorLeftDown { is_energized_from_left_down: is_energized_from_left, is_energized_from_right_up: is_energized_from_right } => *is_energized_from_left || *is_energized_from_right,
